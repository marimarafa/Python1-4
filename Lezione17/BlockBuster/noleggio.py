...
class Noleggio:
    ...
    def rentAMovie(self,film:Film,clientID:str):
        ...
        if self.isAvaible(film):
            ...
            self.list_film.remove(film)
            movies.append(film)
            self.rented_film[clientID] = movies
        else:
            print(f"Non è possibile nolegiare il film {film.getTitle()}!")
        return self.rented_film
    
    def giveBack(self,film:Film,clientID:str,days:int):
        if clientID in self.rented_film and film in self.rented_film[clientID]:
            ...
            print(f'Cliente: {clientID}! La penale da pagare per il film {film.getTitle()} e di {days} euro!')
